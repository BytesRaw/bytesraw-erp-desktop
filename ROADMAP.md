...
- [~] **M7.3** Updates, in this order. An updater that downloads and executes
  ...
  - [~] **M7.3.1** Signing active - the workflow enables itself once the
        ...
        **The client half is done**: SHA-256 is verified on every download, and
        Authenticode is verified whenever the manifest's `signed` field says
        there is a signature to check (`verify_authenticode`, over
        `WinVerifyTrust`). That field is `false` today, so the flip that starts
        fleet-wide enforcement is a manifest edit and not a release. What is
        still outstanding is the certificate itself, which nothing in this
        repository can supply
  - [x] **M7.3.2** An update manifest on a BytesRaw-owned URL rather than the
        ...
        Live at `https://updates.bytesraw.com/erp/stable.json`, served by GitHub
        Pages from the `updates` orphan branch, and regenerated by the release
